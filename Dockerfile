...
# This Dockerfile must be built with a context of the top-level MuxPkgServer.jl directory
WORKDIR /app
...
# Next, copy in full `MuxPkgServer.jl` directory (this is the step that will most often be invalidated)
ADD . /app

# Precompile MuxPkgServer
RUN julia --project=/app -e "using MuxPkgServer"
